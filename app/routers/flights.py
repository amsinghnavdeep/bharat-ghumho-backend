...
from app.data.flights import FLIGHTS, POPULAR_ROUTES, AIRLINES, enrich, extract_code
...
def search_flights(
        ...
        if max_price:
                ...
                if stops is not None:
                            results = [r for r in results if r["stops"] >= stops]
...
def multi_city(data: dict):
    ...
    for i, leg in enumerate(legs):
                ...
                flights = sorted(
                    [enrich(f) for f in FLIGHTS if f["fr"] == fc and f["to"] == tc],
